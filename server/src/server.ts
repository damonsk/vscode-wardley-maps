...
async function validateTextDocument(textDocument: TextDocument): Promise<void> {
	...
	lines.forEach((line, lineIndex) => {
		...
		if (trimmedLine.match(/^ecosystem\s+\w+\s*\[/)) {
			if (problems < settings.maxNumberOfProblems) {
				problems++;
				const ecosystemMatch = trimmedLine.match(
					/^ecosystem\s+(\w+)\s*(\[.*\])/,
				);
				if (ecosystemMatch) {
						...
						if (problems < settings.maxNumberOfProblems) {
							...
							const diagnostic: Diagnostic = {
								...
								range: {
									start: {
										line: lineIndex,
										character: line.indexOf(decorator),
									},
									end: {
										line: lineIndex,
										character: line.indexOf(decorator) + decorator.length,
									},
								},
...
connection.onCompletion(
	(_textDocumentPosition: TextDocumentPositionParams): CompletionItem[] => {
		const d = documents.get(_textDocumentPosition.textDocument.uri);
		...
		const vars: CompletionItem[] = types.map((t) => {
			return {
				label: t,
		...
		const variableCompletes: CompletionItem[] = rawVariables.map((v) => {
			return { label: v, kind: CompletionItemKind.Variable };
		});
		...
		if (_textDocumentPosition.position.character > 0) {
			...
			if (
				...
				currentLine.indexOf('->') ===
					_textDocumentPosition.position.character - 2
			) {
				return variableCompletes;
			}

			if (
				(currentLine.indexOf('component ') > -1 ||
					currentLine.indexOf('market ') > -1 ||
					currentLine.indexOf('submap ') > -1) &&
				...
				const existingDecorators = decoratorSection
					.split(',')
					.map((d) => d.trim())
					.filter((d) => d.length > 0);

				// Filter out already used decorators to avoid duplicates
				const availableDecorators = baseDecorators.filter(
					(decorator) => !existingDecorators.includes(decorator),
				);
			...
			if (currentLine.trim().indexOf('style') === 0) {
				return ['plain', 'colour', 'wardley'].map((v) => {
					return { label: v, kind: CompletionItemKind.Keyword };
				});
			}
